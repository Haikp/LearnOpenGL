...
int main(void)
{
    ...
    unsigned int VAO; //create the object that will store references to attributes for shader usage
    glGenVertexArrays(1, &VAO);
    glBindVertexArray(VAO);

    // create a vertex buffer array
    unsigned int VBO[2]; // location where we will store info about the vertex buffer
    glGenBuffers(2, VBO); // specify that we want to allocate 2 buffers
    glBindBuffer(GL_ARRAY_BUFFER, VBO[0]); // notify OpenGL that the array buffer is in use
    ...
    glBindBuffer(GL_ARRAY_BUFFER, VBO[1]); //use the second buffer that we allocated for VBO
    glBufferData(GL_ARRAY_BUFFER, sizeof(vertices2), vertices2, GL_STATIC_DRAW);

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO[1]); //use the second buffer that we allocated for EBO
