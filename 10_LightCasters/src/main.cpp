...
int main(void)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //for direction light for a large distance source
        ...
        shaderProgram.setVec3("light.specular", glm::vec3(0.1f));

        shaderProgram.setVec3("light.position", camera.getPosition());
        shaderProgram.setVec3("light.direction", camera.getFront());
        shaderProgram.setFloat("light.cutOff", std::cos(glm::radians(12.5)));
        shaderProgram.setFloat("light.outerCutOff", std::cos(glm::radians(17.5)));
